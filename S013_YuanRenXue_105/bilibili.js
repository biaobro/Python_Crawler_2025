...
function encrypt(){
    let prs = [];
    for (let i = 0; i < 4; i++) {
        prs['push'](_0x474032(_0x12eaf3()))
    }
    let _$is = _0x2d5f5b()['toString']()
    let m = _0x474032(_$is)
    prs['push'](m);
    console.log(prs)
    let key = CryptoJS.enc.Utf8.parse(btoa(_$is)['slice'](0, 16));
    let plaintext = CryptoJS.enc.Utf8.parse(prs.toString());
    let encrypted = CryptoJS.AES.encrypt(plaintext, key, {
        mode: CryptoJS.mode.ECB,
        padding: CryptoJS.pad.Pkcs7
    });
    let RM4hZBv0dDon443M = encrypted.toString();
    let data = {
        cookies_m: m,
        cookies_RM4: RM4hZBv0dDon443M,
        params_m: _$is,
        params_f: _0x12eaf3().toString()
    }
    console.log(data);
    return data;
}
